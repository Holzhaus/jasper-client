# -*- coding: utf-8-*-
"""
    Iterates over all the WORDS variables in the modules and creates vocabulary for the respective stt_engine if needed.
"""
...
class PocketsphinxVocabulary(AbstractVocabulary):
    ...
    def decoder_kwargs(self):
        """
        Convenience property to use this Vocabulary with the __init__() method of
        the pocketsphinx.Decoder class.
...
def get_all_phrases(additional_phrases=['MUSIC', 'SPOTIFY']):
    """
    Gets phrases from all modules.

    Arguments:
        additional_phrases -- (optional) a list of additional phrases to add to
                              the result (Default: ['MUSIC', 'SPOTIFY'])
    Returns:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base-dir', action='store', help='the directory in which the vocabulary will be compiled.')
